...
const COLLECTION = "timeline_stops";
...
export async function ensureIndexes() {
  ...
  await coll.createIndex({ stopId: 1 }, { unique: true });
  await coll.createIndex({ order: 1 });
}

export function validate(doc) {
  validateRequiredStrings(doc, ["stopId", "dayRange", "titleKey", "descriptionKey"]);
  if (doc.color !== undefined && typeof doc.color !== "string") {
...
export function prepareForInsert(doc) {
  ...
  return {
    ...doc,
    order: doc.order,
    color: doc.color || "bg-primary border-primary/30 text-white",
...
export async function findByStopId(stopId) {
  const coll = await getCollection();
  return coll.findOne({ stopId });
}
