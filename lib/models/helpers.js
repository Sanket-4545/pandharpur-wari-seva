...
export function requireObjectId(value) {
...
export function generateHelpRequestId(seq) {
  const pad = String(seq).padStart(3, "0");
  return `HELP-2026-${pad}`;
}
...
const SEQUENCE_SUFFIX_REGEX = /-?(\d{1,3})$/;
...
export async function insertOneWithRetry(
  ...
  for (let attempt = 0; attempt < maxAttempts; attempt++) {
    ...
    try {
      ...
      if (error?.code === 11000 && error?.keyPattern && error.keyPattern[idField]) {
        continue;
      }
      throw error;
    }
