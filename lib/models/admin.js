...
export function prepareForInsert(doc) {
  ...
  return {
    ...
    passwordHash: hashPassword(doc.passwordHash),
...
export function comparePassword(admin, plaintextPassword) {
  if (!admin || !admin.passwordHash) return false;
  return comparePwd(plaintextPassword, admin.passwordHash);
}
