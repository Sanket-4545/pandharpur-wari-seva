...
export async function GET(request) {
  try {
    ...
    const [items, total] = await Promise.all([
      Services.getCollection().then(c => c.find(filter).sort({ order: 1 }).skip(skip).limit(limit).toArray()),
      Services.getCollection().then(c => c.countDocuments(filter)),
    ]);
...
export async function POST(request) {
  try {
    ...
    const result = await coll.insertOne(Services.prepareForInsert(body));
