...
import { Search, X, Plus, MapPin, Calendar, ArrowRight, UserX, User, Users, Heart } from 'lucide-react';
...
function MissingPersonCard({ person }) {
  const { t } = useLanguage();
  const displayId = person.caseId || person._id;
  const name = person.name || 'Unknown';
  const normCat = (person.category || '').toLowerCase().replace(/\s+/g, '_');
  const Icon = CATEGORY_ICONS[normCat] || UserX;
  const gradient = CATEGORY_GRADIENTS[person.category] || 'from-red-500 to-rose-600';
  const location = person.lastSeenLocation || '';
  const date = formatDate(person.dateReported);
  ...
  return (
    <div className="group bg-white dark:bg-gray-900 rounded-2xl border border-slate-200 dark:border-gray-800 overflow-hidden shadow-premium hover:shadow-premium-hover transition-all duration-350 hover:-translate-y-1 flex flex-col justify-between">
          ...
          <div className="flex flex-wrap gap-2 mt-2">
            {person.age && (
              <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-lg bg-slate-100 dark:bg-gray-800 text-charcoal-light dark:text-gray-400 text-[10px] font-bold uppercase tracking-wider">
                {person.age} {t('missing_persons_page.age_years')}
              </span>
            )}
            {person.gender && (
              <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-lg bg-slate-100 dark:bg-gray-800 text-charcoal-light dark:text-gray-400 text-[10px] font-bold uppercase tracking-wider">
      ...
      <div className="px-5 pb-5">
        <Link
          href={`/volunteer/missing-persons/${encodeURIComponent(displayId)}`}
        ...
        >
          <span>{t('volunteer_missing_persons.btn_view_details')}</span>
...
export default function VolunteerMissingPersonsPage() {
  const { t } = useLanguage();
  ...
  const fetchPersons = useCallback(async () => {
    try {
      ...
      const res = await fetch(`/api/volunteer/missing-persons?${params.toString()}`);
      ...
      <section className="relative min-h-[30vh] md:min-h-[35vh] flex flex-col justify-center py-16 overflow-hidden bg-gradient-to-br from-red-500 to-rose-600">
        ...
        <Container className="relative z-10 text-center">
          <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full border border-white/20 bg-white/10 backdrop-blur-md text-white text-xs font-semibold mb-6">
            ...
            <span>/</span>
            <span className="font-bold">{t('volunteer_missing_persons.title')}</span>
          </div>
        ...
        <Container>
            ...
            <div className="flex items-center gap-3">
              <div className="relative w-full lg:w-72">
                ...
                <input
                  ...
                  placeholder={t('volunteer_missing_persons.search_placeholder')}
              ...
              <Link
                href="/volunteer/missing-persons/new"
              ...
              >
                ...
                {t('volunteer_missing_persons.btn_add_new')}
              </Link>
            </div>
          </div>
          ...
          {loading ? (
            ...
            <div className="text-center py-16 bg-white dark:bg-gray-900 border border-slate-200 dark:border-gray-800 rounded-2xl p-8 max-w-md mx-auto">
              <div className="w-16 h-16 mx-auto mb-4 rounded-full bg-slate-100 dark:bg-gray-800 flex items-center justify-center">
                <UserX className="w-7 h-7 text-slate-400 dark:text-gray-500" />
              </div>
              <p className="text-lg font-heading font-extrabold text-charcoal dark:text-white">
                {t('volunteer_missing_persons.no_results')}
              </p>
              <p className="text-sm text-charcoal-light dark:text-gray-400 mt-2">
                {t('volunteer_missing_persons.no_results_desc')}
              </p>
            </div>
          ) : (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8">
              {persons.map((person) => (
                <MissingPersonCard key={person.caseId || person._id} person={person} />
              ))}
            </div>
          )}
          ...
          {!loading && !error && persons.length > 0 && (
            <p className="text-center text-xs text-slate-400 dark:text-gray-500 mt-10 font-semibold">
              {t('volunteer_missing_persons.persons_count', { count: persons.length, plural: persons.length !== 1 ? 's' : '' })}
            </p>
          )}
        </Container>
      </section>
